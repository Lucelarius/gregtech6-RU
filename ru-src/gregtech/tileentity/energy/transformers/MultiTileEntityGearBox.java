...
public class MultiTileEntityGearBox extends TileEntityBase07Paintable implements ITileEntityEnergy, ITileEntityRunningActively, ITileEntitySwitchableOnOff, IMTE_GetOreDictItemData, IMTE_AddToolTips {
	...
	static {
		LH.add("gt.tooltip.gearbox.custom.1", "Шестерни сцепляются не правильно!");
		LH.add("gt.tooltip.gearbox.custom.2", "Используйте гаечный ключ, для установки шестерней из вашего инвентаря");
		LH.add("gt.tooltip.gearbox.custom.3", "Используйте разводной ключ, чтобы изменить направление оси");
	}
	...
	public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer, List<String> aChatReturn, IInventory aPlayerInventory, boolean aSneaking, ItemStack aStack, byte aSide, float aHitX, float aHitY, float aHitZ) {
		...
		if (aTool.equals(TOOL_wrench)) {
			...
			if (aChatReturn != null) aChatReturn.add("В вашем инвентаре нет снаряжения соответствующего материала!");
			return 0;
		}
		...
		if (aTool.equals(TOOL_tachometer)) {
			if (aChatReturn != null) {
				if (FACE_CONNECTED[0][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][0]) aChatReturn.add(FACE_CONNECTED[0][mInputtedSides] ? "Принимает снизу"    : "Отдает снизу");
				if (FACE_CONNECTED[1][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][1]) aChatReturn.add(FACE_CONNECTED[1][mInputtedSides] ? "Принимает сверху"   : "Отдает сверху");
				if (FACE_CONNECTED[2][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][2]) aChatReturn.add(FACE_CONNECTED[2][mInputtedSides] ? "Принимает с севера" : "Отдает с севера");
				if (FACE_CONNECTED[3][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][3]) aChatReturn.add(FACE_CONNECTED[3][mInputtedSides] ? "Принимает с юга"    : "Отдает с юга");
				if (FACE_CONNECTED[4][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][4]) aChatReturn.add(FACE_CONNECTED[4][mInputtedSides] ? "Принимает с запада" : "Отдает с запада");
				if (FACE_CONNECTED[5][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][5]) aChatReturn.add(FACE_CONNECTED[5][mInputtedSides] ? "Принимает с востока": "Отдает с востока");
		...
		if (aTool.equals(TOOL_magnifyingglass)) {
			mGearsWork = checkGears();
			if (aChatReturn != null) aChatReturn.add(mGearsWork ? mJammed ? "Шестерни сцепились правильно, но их заклинило!" : "Шестерни сцепились правильно." : "Шестерни сцепились неправильно");
			return 1;
		}
		return 0;
	}
	...
	public byte getRotations(byte aSide, boolean aNegative) {
		...
		if (AXIS_XYZ[(mAxleGear >>> 6) & 3][aSide]) {
			...
			ERR.print("Что-то пошло не так со свободной осью внутри коробки передач " + getCoords() + " при получении мощности со стороны: " + aSide);
		...
		ERR.print("Что-то пошло не так с коробкой передач " + getCoords() + " при получении мощности со стороны: " + aSide + ", хотя на этой стороне нет ни шестерни, ни оси");
		// Returning the current Rotation Data to make sure nothing breaks too badly.
		return mRotationData;
	}
