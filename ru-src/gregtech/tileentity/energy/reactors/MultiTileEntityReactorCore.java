...
public abstract class MultiTileEntityReactorCore extends TileEntityBase10FacingDouble implements ITileEntityServerTickPost, IFluidHandler, ITileEntityTapAccessible, ITileEntityFunnelAccessible, ITileEntityRunningActively, ITileEntitySwitchableOnOff, IMTE_GetCollisionBoundingBoxFromPool, IMTE_OnEntityCollidedWithBlock {
	...
	static {
		LH.add("gt.tooltip.reactorcore.1", "Доступ к слотам реактора можно получить сверху и снизу.");
		LH.add("gt.tooltip.reactorcore.2", "Доступ к слоту реактора возможен только тогда, когда слот или реактор выключены.");
		LH.add("gt.tooltip.reactorcore.3", "Первичная облицовка выделяет горячую охлаждающую жидкость.");
		LH.add("gt.tooltip.reactorcore.4", "Вторичная облицовка испускает холодную охлаждающую жидкость, когда она заполнена более чем наполовину.");
		LH.add("gt.tooltip.reactorcore.5", "Выбор охлаждающей жидкости может иметь особые эффекты.");
	}
	...
	public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer, List<String> aChatReturn, IInventory aPlayerInventory, boolean aSneaking, ItemStack aStack, byte aSide, float aHitX, float aHitY, float aHitZ) {
		...
		if (aTool.equals(TOOL_softhammer)) {
			mStopped = !mStopped;
			if (aChatReturn != null) {
				aChatReturn.add(mStopped?"Реакторный блок ВЫКЛЮЧЕН":"Реакторный блок ВКЛЮЧЕН");
			}
			return 10000;
		}
		if (aTool.equals(TOOL_thermometer)) {
			if (aChatReturn != null) {
				aChatReturn.add("Уровень тепла: " + (oEnergy <= 0 ? "Ничего" : oEnergy + " HU"));
			}
			return 10000;
		}
		if (aTool.equals(TOOL_magnifyingglass)) {
			if (aChatReturn != null) {
				aChatReturn.add("Ввод: "  + mTanks[0].content());
				aChatReturn.add("Вывод: " + mTanks[1].content());
				aChatReturn.add(mStopped?"Реакторный блок ВЫКЛЮЧЕН":"Реакторный блок ВКЛЮЧЕН");
			}
			return 1;
		}
