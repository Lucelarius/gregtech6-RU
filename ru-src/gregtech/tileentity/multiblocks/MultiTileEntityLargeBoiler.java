...
public class MultiTileEntityLargeBoiler extends TileEntityBase10MultiBlockBase implements ITileEntityEnergy, ITileEntityGibbl, ITileEntityEnergyDataCapacitor, IMultiBlockEnergy, IMultiBlockFluidHandler, IFluidHandler, IMTE_RemovedByPlayer {
	...
	static {
		LH.add("gt.tooltip.multiblock.largeboiler.1", "3x3 Основа из теплопередатчиков");
		LH.add("gt.tooltip.multiblock.largeboiler.2", "Полая 3x3x3 из того же материала, что и главный блок");
		LH.add("gt.tooltip.multiblock.largeboiler.3", "Главный блок распологается по центру сбоку снизу и направлен наружу");
		LH.add("gt.tooltip.multiblock.largeboiler.4", "Ввод возможен только в нижнем слое котла");
	}
	...
	public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
		...
		aList.add(Chat.RED      + LH.get(LH.ENERGY_OUTPUT)          + ": " + Chat.WHITE + UT.Code.units(mOutput, 10000, mEfficiency, F) + " " + TD.Energy.STEAM.getLocalisedChatNameLong()      + Chat.WHITE + "/t (Pipe Holes / Отверстия для труб)");
	...
	public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer, List<String> aChatReturn, IInventory aPlayerInventory, boolean aSneaking, ItemStack aStack, byte aSide, float aHitX, float aHitY, float aHitZ) {
		...
		if (aTool.equals(TOOL_thermometer)) {
			if (aChatReturn != null) aChatReturn.add("Юнитов накопленного тепла: " + mEnergy + " / " + mCapacity + " HU");
			return 10000;
		}
		return 0;
	}
	...
	public void onMagnifyingGlass2(List<String> aChatReturn) {
		if (mEfficiency < 10000) {
			aChatReturn.add("Кальцификация: " + LH.percent(10000 - mEfficiency) + "%");
		} else {
			aChatReturn.add("В этом котле нет кальцификации");
		}
		aChatReturn.add(mTanks[0].content("ВНИМАНИЕ: НЕТ ВОДЫ!!!"));
	}
