...
public class MultiTileEntityCrucible extends TileEntityBase10MultiBlockBase implements ITileEntityCrucible, ITileEntityEnergy, ITileEntityGibbl, ITileEntityWeight, ITileEntityTemperature, ITileEntityMold, ITileEntityServerTickPost, ITileEntityEnergyDataCapacitor, IMultiBlockEnergy, IMultiBlockInventory, IMultiBlockFluidHandler, IFluidHandler {
	...
	static {
		LH.add("gt.tooltip.multiblock.crucible.1", "Полая 3x3x3 из стен с отверстием сверху.");
		LH.add("gt.tooltip.multiblock.crucible.2", "Главный блок снизу в центре.");
		LH.add("gt.tooltip.multiblock.crucible.3", "Вход энергии в нижнем слое, для предметов в верхнем.");
		LH.add("gt.tooltip.multiblock.crucible.4", "Формы можно использовать на втором слое стен");
		LH.add("gt.tooltip.multiblock.crucible.5", "Входящая KU превратится в воздух для производства стали");
	}
	...
	public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
		...
		aList.add(Chat.CYAN     + LH.get(LH.CONVERTS_FROM_X) + " 1 " + TD.Energy.HU.getLocalisedNameShort() + " " + LH.get(LH.CONVERTS_TO_Y) + " +1 K " + LH.get(LH.CONVERTS_PER_Z) + " "+ KG_PER_ENERGY + "kg (требуется не менее "+getEnergySizeInputMin(TD.Energy.HU, SIDE_ANY)+" Юнитов в тик!)");
