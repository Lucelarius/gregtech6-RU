...
public class ANY {
	...
	protected static void init() {
		...
		Amber           .stealLooks(MT.Amber          ).steal(MT.Amber          ).setLocal("Янтарь"                                                     ).put(CRYSTAL, VALUABLE                                      ).addReRegistrationToThis(MT.AmberGolden, MT.AmberDominican);
		...
		MagicIron       .stealLooks(MT.Manasteel      ).steal(MT.Manasteel      ).setLocal("Магическая сталь"   ).setAllToTheOutputOf(MT.Fe             ).put(SMITHABLE, MELTING, MAGICAL                            ).addReRegistrationToThis(MT.Manasteel, MT.Thaumium, MT.DarkThaumium, MT.SpectreIron, MT.FierySteel, MT.MeteoflameSteel);
