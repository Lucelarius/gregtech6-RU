...
public class LanguageHandler {
	...
	public static String getLocalName(OreDictPrefix aPrefix, OreDictMaterial aMaterial) {
		...
		if (aMaterial.mID >= 0 && aMaterial.mID < 10 && aPrefix.contains(TD.Prefix.ORE)) return APRIL_FOOLS ? "Руда Шредингера" : "Неопознанная руда";
		...
		if (aMaterial == MT.AncientDebris) {
			...
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return "Порошок (" + aMaterial.mNameLocal + ")";
		...
		if (aMaterial == MT.STONES.MoonTurf) {
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return "Торф (" + aMaterial.mNameLocal + ")";
		...
		if (aMaterial == MT.Peridot) {
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return aPrefix.mMaterialPre + "Оливин";
		} else
		if (aMaterial == MT.Craponite) {
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return aPrefix.mMaterialPre + "Крапонит)";
		} else
		if (aMaterial == MT.Wheat) {
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return "Мука (" + aMaterial.mNameLocal + ")";
		} else
		if (aMaterial == MT.Oat) {
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return "Мука (" + aMaterial.mNameLocal + ")";
		} else
		...
		if (aMaterial == MT.Rice) {
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return "Рис";
		} else
		if (aMaterial == MT.Ice) {
			...
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return aPrefix.mMaterialPre + "Дробленый лед)";
		} else
		if (aMaterial == MT.WoodTreated) {
			...
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return "Целлюлоза (" + aMaterial.mNameLocal + ")";
		...
		if (aMaterial == MT.Bark) {
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return aPrefix.mMaterialPre + "Кора)";
		} else
		...
		if (aMaterial == MT.Bone) {
			...
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return "Костная мука";
		...
		if (aMaterial == MT.MeatRaw) {
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return "Мясной фарш (" + aMaterial.mNameLocal + ")";
		} else
		...
		if (aMaterial == MT.KCl) {
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return aPrefix.mMaterialPre + "Каменная соль";
		} else
		...
		if (aMaterial == MT.Black || aMaterial == MT.Red || aMaterial == MT.Green || aMaterial == MT.Brown || aMaterial == MT.Blue || aMaterial == MT.Purple || aMaterial == MT.Cyan || aMaterial == MT.LightGray || aMaterial == MT.Gray || aMaterial == MT.Pink || aMaterial == MT.Lime || aMaterial == MT.Yellow || aMaterial == MT.LightBlue || aMaterial == MT.Magenta || aMaterial == MT.Orange || aMaterial == MT.White) {
			...
			if (aPrefix.contains(TD.Prefix.DUST_BASED))                             return "Краситель (" + aMaterial.mNameLocal + ")";
		} else
