...
public class BlockLongDistPipe extends BlockBaseMachineUpdate {
	...
	public BlockLongDistPipe(String aUnlocalised, IIconContainer[] aIcons, long[] aTemperatures) {
		...
		LH.add(aUnlocalised+".0" , "Предметная труба дальней передачи");
		for (int i = 1; i < maxMeta(); i++) LH.add(aUnlocalised+"."+i, "Жидкостная труба дальней передачи ("+mTemperatures[i]+" K)");
	}
