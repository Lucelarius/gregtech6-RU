...
public class Behavior_Gun extends AbstractBehaviorDefault {
	...
	//  MinecraftForge.EVENT_BUS.post(new AttackEntityEvent(aPlayer, aTarget));
		...
		DamageSource tDamageSource = DamageSources.getCombatDamage("player", tPlayer, DamageSources.getDeathMessage(aPlayer, aTarget, (tData!=null&&tData.validMaterial() ? "[VICTIM] был(а) убит(а) [KILLER] выстрелом из " + tData.mMaterial.mMaterial.getLocal() : "[VICTIM] был(а) застрелен(а) [KILLER]")), F).setProjectile();
