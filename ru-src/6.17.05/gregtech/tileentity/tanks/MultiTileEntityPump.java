...
public class MultiTileEntityPump extends TileEntityBase09FacingSingle implements IFluidHandler, ITileEntityEnergy, ITileEntityRunningActively, ITileEntitySwitchableOnOff {
	...
	static {
		LH.add("gt.tooltip.pump.1", "Можно использовать для выкачивания больших объемов жидкости");
		LH.add("gt.tooltip.pump.2", "Не подходит для выкачивания бесконечных источников жидкости!");
		LH.add("gt.tooltip.pump.3", "Для этого используйте дренаж!");
	}
	...
	public void overload(long aSize, TagData aEnergyType) {
		if (mExplosionPrevention < 100) {
			if (mTimer < 100) DEB.println("Машина перегружена при запуске из-за: " + aSize + " " + aEnergyType.getLocalisedNameLong());
