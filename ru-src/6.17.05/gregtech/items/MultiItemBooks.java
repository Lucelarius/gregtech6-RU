...
public class MultiItemBooks extends MultiItemRandomWithCompat {
	...
	public void addItems() {
		...
		BooksGT.BOOK_REGISTER.put(addItem(32002, "Словарь материалов"        , "Книга о материале"                                         , OD.bookWrittenSmall, TD.Creative.HIDDEN, TC.stack(TC.COGNITIO, 2), TICKS_PER_SMELT  , new OreDictItemData(MT.Paper, U * 3)), (byte)11); BooksGT.BOOKS_NORMAL.add(last());
		BooksGT.BOOK_REGISTER.put(addItem(32003, "Словарь материалов"        , "Книга о материале"                                         , OD.bookWrittenBig  , TD.Creative.HIDDEN, TC.stack(TC.COGNITIO, 4), TICKS_PER_SMELT*2, new OreDictItemData(MT.Paper, U * 6)), (byte)11); BooksGT.BOOKS_NORMAL.add(last());
		...
		BooksGT.BOOK_REGISTER.put(addItem(32700, "Аспектономикон"            , "Аспекты и элементы, на которых они находятся"              , TC.stack(TC.COGNITIO, 9), TICKS_PER_SMELT, new OreDictItemData(MT.Paper, U * 9), new Behavior_Unlock_Item_Aspects(MD.MC, MD.EtFu, MD.NeLi, MD.EnLi, MD.NePl, MD.GaSu, MD.GaNe, MD.GaEn, MD.WdSt, MD.TFC, MD.TFCP, MD.HaC, MD.GrC, MD.GrC_Apples, MD.GrC_Bamboo, MD.GrC_Bees, MD.GrC_Cellar, MD.GrC_Fish, MD.GrC_Hops, MD.GrC_Grapes, MD.GrC_Milk, MD.GrC_Rice, MD.Salt, MD.BoP, MD.EB, MD.EBXL, MD.BWM, MD.BbLC, MD.SD, MD.BTRS, MD.JABBA, MD.CARP, MD.CHSL, MD.ZTONES)), (byte)13); BooksGT.BOOKS_ENCHANTED.add(last()); IL.Book_Aspectonomicon.set(last());

		BooksGT.BOOK_REGISTER.put(addItem(32765, "Пыльное руководство"       , "Добыча: Какое-то случайное руководство или что-то подобное", TC.stack(TC.COGNITIO, 3), TICKS_PER_SMELT, new OreDictItemData(MT.Paper, U * 3), new Behavior_Drop_Loot("gt.books"   )), (byte)53); BooksGT.BOOKS_NORMAL.add(last()); IL.Book_Loot_Guide.set(last());
		BooksGT.BOOK_REGISTER.put(addItem(32766, "Пыльный словарь материалов", "Добыча: Книга о случайном материале"                       , TC.stack(TC.COGNITIO, 3), TICKS_PER_SMELT, new OreDictItemData(MT.Paper, U * 3), new Behavior_Drop_Loot("gt.matdicts")), (byte)11); BooksGT.BOOKS_NORMAL.add(last()); IL.Book_Loot_MatDict.set(last());
