	...
	public void addAdditionalToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
		...
		if (tMaxDamage > 0 && tStats != null) {
			if (tMat1 == MT.NULL) {
				...
				aList.add(LH.Chat.WHITE + "Урон в ближнем бою: +" + LH.Chat.BLUE + (tCombat * TFC_DAMAGE_MULTIPLIER) + LH.Chat.RED + " (= " + (TFC_DAMAGE_MULTIPLIER > 1 ? ((tCombat+1)*(TFC_DAMAGE_MULTIPLIER/2.0)) + ")" : ((tCombat+1)/2) + "\u2764)"));
