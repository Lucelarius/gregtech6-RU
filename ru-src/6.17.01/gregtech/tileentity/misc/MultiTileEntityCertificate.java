...
public class MultiTileEntityCertificate extends TileEntityBase09FacingSingle implements IMTE_OnRegistration, IMTE_OnDespawn, IMTE_GetLifeSpan, IMTE_IgnorePlayerCollisionWhenPlacing, IMTE_OnServerLoad, IMTE_OnServerSave, IMTE_SetBlockBoundsBasedOnState, IMTE_GetCollisionBoundingBoxFromPool, IMTE_GetSelectedBoundingBoxFromPool {
	...
	static {LH.add("gt.tooltip.certificate", "Сертифицированная поддержка GregTech");}
	...
	public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
		if (isServerSide()) {
			...
			if (mGold  ) {UT.Entities.sendchat(aPlayer, "Этот сертификат принадлежит" + LH.Chat._YELLOW + getCustomName()); return T;}
			if (mSilver) {UT.Entities.sendchat(aPlayer, "Этот сертификат принадлежит" + LH.Chat._CYAN   + getCustomName()); return T;}
			UT.Entities.sendchat(aPlayer, "Данный cертификат недействителен!");
			return T;
		}
		return T;
	}
