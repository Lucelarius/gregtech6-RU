...
public class MultiTileEntityLogisticsCore extends TileEntityBase10MultiBlockBase implements ITileEntityEnergy, ITileEntityLogistics, ITileEntityServerTickPre, ITileEntityEnergyDataCapacitor, IMultiBlockEnergy, IMultiBlockFluidHandler, IFluidHandler {
	...
	static {
		LH.add("gt.tooltip.multiblock.logisticscore.1" , "5x5x5 Рама из 44 оцинкованных стальных стен (Подача энергии здесь)");
		LH.add("gt.tooltip.multiblock.logisticscore.2" , "3x3x3 Ядро из 27 модулей мультиблочного процессора (Настраивается)");
		LH.add("gt.tooltip.multiblock.logisticscore.3" , "6 стен 3х3 из 53 вентиляционных модулей");
		LH.add("gt.tooltip.multiblock.logisticscore.4" , "Главный блок по центру с любой стороны направленный наружу");
		LH.add("gt.tooltip.multiblock.logisticscore.5" , "Хотябы по 1 модулю каждого типа (Или используйте универсальный)");
		LH.add("gt.tooltip.multiblock.logisticscore.6" , "Вы можете заменить процессорные модули на стены, если не можете позволить себе так много");
		LH.add("gt.tooltip.multiblock.logisticscore.7" , "Логический модуль увеличивает количество операций на 1");
		LH.add("gt.tooltip.multiblock.logisticscore.8" , "Управляющий модуль увеличивает максимальный радиус сети на 1м (Cubic AoE)");
		LH.add("gt.tooltip.multiblock.logisticscore.9" , "Модуль памяти увеличивает размер буффера на 1 стак или 16000L");
		LH.add("gt.tooltip.multiblock.logisticscore.10", "Конвертирующий модуль увеличивает пропускную способность на 1 стак или 16000L");
	}
	...
	public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
		...
		aList.add(Chat.GREEN    + LH.get(LH.ENERGY_INPUT) + ": " + Chat.WHITE + "256 до 1024 " + mEnergyTypeAccepted.getLocalisedChatNameShort() + Chat.WHITE + "/t");
	...
	public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
		if (isServerSide() && aPlayer != null && checkStructure(F)) {
			...
			tChat.add("Энергия: " + mEnergy + " EU");
			tChat.add("Потребление: " + (20+mCPU_Logic+mCPU_Control+mCPU_Storage+mCPU_Conversion) + " EU/t, плюс еще за каждый перемещенный предмет/жидкость");
			tChat.add(mCPU_Logic + " логический модуль");
			tChat.add(mCPU_Control + " управляющий модуль (Расстояние: "+(2+mCPU_Control)+"m, Cubic AoE)");
			tChat.add(mCPU_Storage + " модуль хранения (Заметка: На данный момент бесполезно)");
			tChat.add(mCPU_Conversion + " конвертирующий модуль");
