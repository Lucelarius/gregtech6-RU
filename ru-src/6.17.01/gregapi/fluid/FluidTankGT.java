...
public class FluidTankGT implements IFluidTank {
	...
	public String content() {return content("Пусто");}
	public String content(String aEmptyMessage) {return  mFluid == null ? aEmptyMessage                     : UT.Code.makeString(amount()) + " L of " + name(T) + " (" + (FL.gas(mFluid) ? "Газ" : "Жидкость") + ")";}
	public String contentcap() {return mFluid == null ? "Емкость: " + UT.Code.makeString(mCapacity) + " L" : UT.Code.makeString(amount()) + " L of " + name(T) + " (" + (FL.gas(mFluid) ? "Газ" : "Жидкость") + "); Макс: "+UT.Code.makeString(capacity())+" L)";}
