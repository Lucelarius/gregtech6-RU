		...
		try {
			...
			if (tData != null) {
				...
				if (aEvent.showAdvancedItemTooltips) {
					...
					aEvent.toolTip.add(LH.Chat.DGRAY + "Включите режим F3+H для получения информации о содержащихся материалах.");
				}
				if (ST.isGT(aItem) && tData.validData()) {
					if (tData.mMaterial.mMaterial.mOriginalMod == null) {
						aEvent.toolTip.add(LH.Chat.BLUE + "Материал из неизвестного мода");
					} else if (tData.mMaterial.mMaterial.mOriginalMod == MD.MC) {
						aEvent.toolTip.add(LH.Chat.BLUE + "Ванильный материал");
						...
						if (tData.mMaterial.mMaterial.mID > 0 && tData.mMaterial.mMaterial.mID < 8000) {
							aEvent.toolTip.add(LH.Chat.BLUE + "Материал из периодической таблицы элементов");
						} else {
							aEvent.toolTip.add(LH.Chat.BLUE + "Случайный материал созданный Greg API");
						}
					} else {
						aEvent.toolTip.add(LH.Chat.BLUE + "Материал из " + tData.mMaterial.mMaterial.mOriginalMod.mName);
					}
				}
			}
