...
public class LanguageHandler {
	...
	public static String getLocalName(OreDictPrefix aPrefix, OreDictMaterial aMaterial) {
		...
		if (aPrefix == OP.crateGtDust     || aPrefix == OP.crateGt64Dust     || aPrefix == OP.blockDust    ) {
			if (aMaterial == MT.WoodTreated)                                                                 return aPrefix.mMaterialPre + "целлюлозы (" + aMaterial.mNameLocal + ")";
			if (ANY.Plastic.mToThis.contains(aMaterial) || ANY.Rubber.mToThis.contains(aMaterial))           return aPrefix.mMaterialPre + "целлюлозы (" + aMaterial.mNameLocal + ")";
			if (aMaterial.contains(TD.Properties.WOOD))                                                      return aPrefix.mMaterialPre + "целлюлозы (" + aMaterial.mNameLocal + ")";
		...
		if (aMaterial == MT.Black || aMaterial == MT.Red || aMaterial == MT.Green || aMaterial == MT.Brown || aMaterial == MT.Blue || aMaterial == MT.Purple || aMaterial == MT.Cyan || aMaterial == MT.LightGray || aMaterial == MT.Gray || aMaterial == MT.Pink || aMaterial == MT.Lime || aMaterial == MT.Yellow || aMaterial == MT.LightBlue || aMaterial == MT.Magenta || aMaterial == MT.Orange || aMaterial == MT.White) {
			...
			if (aPrefix.mNameInternal.startsWith("dust"))                           return aPrefix.mMaterialPre + aMaterial.mNameLocal + " Dye1";
		} else
