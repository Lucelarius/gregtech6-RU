		...
		if (aTool.equals(TOOL_magnifyingglass)) {
			if (aChatReturn != null) {
				if (aData.mNBTs[aCoverSide] == null) {
					aChatReturn.add("Нет набора фильтров! (Приоритет: " + aData.mValues[aCoverSide] + ")");
					...
					if (ST.invalid(tStack)) {
						...
						aChatReturn.add("Экспорт: " + LH.Chat.CYAN + ST.regName(tStack) + LH.Chat.GRAY + " ; " + LH.Chat.CYAN + ST.meta_(tStack) + " (Приоритет: " + aData.mValues[aCoverSide] + ")");
					}
				}
				int tTargetSize = ((aData.mValues[aCoverSide] >> 2) & 127);
				if (tTargetSize == 0) {
					aChatReturn.add("Переменный целевой размер стака");
				} else {
					aChatReturn.add("Целевой размер стака: " + tTargetSize);
				}
			}
			return 1;
		}
		...
		if (aPlayer instanceof EntityPlayer && aData.mTileEntity.isServerSide()) {
			if (aData.mNBTs[aCoverSide] == null || !aData.mNBTs[aCoverSide].hasKey("gt.filter.item")) {
				...
				if (ST.valid(tStack)) {
					...
					UT.Entities.sendchat(aPlayer, "Экспорт: " + LH.Chat.CYAN + ST.regName(tStack) + LH.Chat.GRAY + " ; " + LH.Chat.CYAN + ST.meta_(tStack));
				}
			}
		}
		return T;
